...
Hashing is the cryptographic backbone of the whole suite, so it lives
alone in a module small enough to audit at a glance. Files are read in
fixed-size chunks rather than loaded whole: exhibits can be multi-GB
video exports, and memory usage stays constant regardless.
"""
...
def sha256_file(path: Path) -> str:
    ...
    Symlinks are followed by ``open`` itself: the digest covers the
    content the link resolves to, which is what an examiner opening the
    file would see.
    """
